            ...
            <div id='fundo-modal-adicionar'>
                <div id='modal-adicionar'>
                    <div id='titulo-modal' style={{ backgroundColor: titleBackgroundColor }}>
                        ...
                        <button id='fechar-modal' onClick={setCloseModal}>X</button>
                    </div>

                    <div id='input-nomeTarefa'>
                        <h4 id='titulo-nomeTarefa'>Nome da tarefa</h4>
                        <input type="text" value={taskName} onChange={handleTaskNameChange} maxLength={30} />
                        <div id="contador-titulo">{taskName.length}/30</div>
                    </div>

                    <div id='input-tempo'>
                        <h4 id='titulo-dataTermino'>Data de término</h4>
                        <input id='tempo-termino' type="date" placeholder='Data de término' value={endDate} onChange={handleEndDateChange} />
                    </div>

                    <div id='textarea-descricao'>
                        <h4 id='titulo-descricao'>Descrição da tarefa</h4>
                        <textarea value={description} onChange={handleDescriptionChange} maxLength={100}></textarea>
                        <div id="contador">{description.length}/100</div>
                    </div>

                    <div id='btns-modal'>
                        <button id='btn-adicionar' onClick={handleAddTask}>Adicionar Tarefa</button>
                    </div>

                    <div id='title-background-color-menu'>
                        <h4 id='titulo-cor'>Coloração</h4>
                        <input type="color" value={titleBackgroundColor} onChange={(event) => handleTitleBackgroundColorChange(event.target.value)} />
                    </div>
